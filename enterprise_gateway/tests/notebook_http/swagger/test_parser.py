...
class TestSwaggerAPICellParser(unittest.TestCase):
    ...
    def test_basic_is_api_cell(self):
        ...
        parser = SwaggerCellParser(comment_prefix='#', notebook_cells=[{"source":'```\n{"swagger":"2.0", "paths": {"": {"post": {"operationId": "foo", "parameters": [{"name": "foo"}]}}}}\n```\n'}]) # noqa
        self.assertTrue(parser.is_api_cell('#operationId:foo'), 'API cell was not detected with ' +
                        str(parser.kernelspec_operation_indicator))
        self.assertTrue(parser.is_api_cell('# operationId:foo'), 'API cell was not detected with ' +
                        str(parser.kernelspec_operation_indicator))
        self.assertTrue(parser.is_api_cell('#operationId: foo'), 'API cell was not detected with ' +
                        str(parser.kernelspec_operation_indicator))
    ...
    def test_basic_is_api_response_cell(self):
        ...
        parser = SwaggerCellParser(comment_prefix='#', notebook_cells=[{"source":'```\n{"swagger":"2.0", "paths": {"": {"post": {"operationId": "foo", "parameters": [{"name": "foo"}]}}}}\n```\n'}]) # noqa
        self.assertTrue(parser.is_api_response_cell('#ResponseInfo operationId:foo'),
                        'Response cell was not detected with ' + str(parser.kernelspec_operation_response_indicator))
        self.assertTrue(parser.is_api_response_cell('# ResponseInfo operationId:foo'),
                        'Response cell was not detected with ' + str(parser.kernelspec_operation_response_indicator))
        self.assertTrue(parser.is_api_response_cell('# ResponseInfo  operationId: foo'),
                        'Response cell was not detected with ' + str(parser.kernelspec_operation_response_indicator))
        self.assertTrue(parser.is_api_response_cell('#ResponseInfo operationId: foo'),
                        'Response cell was not detected with ' + str(parser.kernelspec_operation_response_indicator))
        self.assertFalse(parser.is_api_response_cell('# operationId: foo'),
